  ...
  useEffect(() => {
    const fetchVideo = async () => {
      ...
      try {
        ...
        // View increment
        await fetch(`http://localhost:5000/api/videos/${id}/view`, { method: 'POST' });

        // Related videos
        const relRes  = await fetch('http://localhost:5000/api/videos');
            ...
            <div className="vp-uploader">
              <div className="vp-avatar">
                {video.uploader?.profilePic ? (
                  <img src={`http://localhost:5000${video.uploader.profilePic}`} alt="" />
                ) : (
