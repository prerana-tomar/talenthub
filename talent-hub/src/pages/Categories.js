...
const Categories = () => {
  ...
  const fetchCategoryStats = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/videos/category-stats');
  ...
  const fetchVideosByCategory = async (cat) => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/api/videos?category=${cat}`);
      const data = await res.json();
            ...
            {videos.map((video) => (
              ...
              >
                <div className="video-thumb">
                  <video src={`http://localhost:5000${video.videoUrl}`} muted />
